...
final updatePrompt =
    // The leading newline matches the exact format produced by the runner.
...
void main() {
  group('DartSlackCommandRunner', () {
    ...
    test(
      'shows error message when failed to check for updates',
      () async {
        when(
          () => pubUpdater.getLatestVersion(any()),
    ...
    test(
      'does not show update message when the shell '
    ...
    test(
      'does not show update message when using update command',
      () async {
        when(
          () => pubUpdater.getLatestVersion(any()),
        ).thenAnswer((_) async => latestVersion);
