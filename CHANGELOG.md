...
- Authenticated commands fall back to the `SLACK_TOKEN` environment variable when no `~/.dart_slack/credentials.json` is present, so the CLI works in non-interactive contexts (CI, containers, agent workspaces) without an interactive `login`.
